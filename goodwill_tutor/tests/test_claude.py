...
sent = " ".join(p.get("text", "") for t in state["seen"] for p in t["parts"])
r.check("the earlier answer is not sent again in full", 'class="final-ans"' in sent, False)
r.check("only a summary of it goes", "Illustration 6: " in sent and "Final answer" in sent)
